...
def get_random_services(request, input_num):
    ...
    while input_num != len(new_list):
        ...
        service = Service.objects.annotate(is_dib=Exists(Dib.objects.filter(users__pk=request.user.id, service_id=OuterRef('pk')))).annotate(avg_reviews=Avg('review__score')).filter(pk=pk).first()
