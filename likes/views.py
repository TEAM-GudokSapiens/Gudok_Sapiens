...
def likes_dibs(request, pk):
    new_dib, created = Dib.objects.get_or_create(
        users_id=request.user.id, service_id=pk)
...
def likes_helps(request, pk):
    new_help, created = Help.objects.get_or_create(
        users_id=request.user.id, review_id=pk)
    # created==True면 이번에 만들었음.
    # created ==False -> not created ==True는 이미 만들어져서 삭제하러 가는 것.
    if not created:
        new_help.delete()
...
def dibs_ajax(request):
    ...
    service_id = req['id']
    new_dib, created = Dib.objects.get_or_create(
        users_id=request.user.id, service_id=service_id)
    # created==True면 이번에 만들었음.
    # created ==False -> not created ==True는 이미 만들어져서 삭제하러 가는 것.
    if not created:
        new_dib.delete()
...
def help_ajax(request):
    ...
    review_id = req['id']
    new_help, created = Help.objects.get_or_create(
        users_id=request.user.id, review_id=review_id)
    # created==True면 이번에 만들었음.
    # created ==False -> not created ==True는 이미 만들어져서 삭제하러 가는 것.
    if not created:
        new_help.delete()
